...
export const Player: React.FC = () => {
    ...
    const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
        const armorColor = isImmortalityActive ? '#ffd700' : '#ff0055'; // Hot pink neon graffiti style
        const glowColor = isImmortalityActive ? '#ffffff' : '#00ffcc';

        return {
            armorMaterial: new THREE.MeshToonMaterial({ color: armorColor }),
            jointMaterial: new THREE.MeshToonMaterial({ color: '#111111' }),
